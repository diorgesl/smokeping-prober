...
- Routers (`routers:` top-level key, remote ping via Huawei VRP over SSH — see `SMOKEPING-PROBER_README.md`) are managed through `/api/routers` (`RouterInput`/`LinkInput`, keyed by router `name`). A target with `router:` set is pinged by that router once per link; `target_id()` appends the router to the hash only for remote targets, so local target ids are unchanged. Renaming a router cascades to its targets; deleting a router still used by targets, or removing a link a target lists in `links:`, returns 409.
- Remote series carry a `link` label: `PrometheusClient.metrics()` groups by `link` too, keys results by `(host, title, category, remote)`, returns per-link breakdown in `metrics["links"]`, and `metric_status()` uses the worst link's loss.
