...
- Routers (`routers:` top-level key, remote ping via Huawei VRP over SSH — see `SMOKEPING-PROBER_README.md`) are managed through `/api/routers` (`RouterInput`/`LinkInput`, keyed by router `name`). A target with `router:` set is pinged by that router from every one of its links (targets have no `links:` key); `target_id()` appends the router to the hash only for remote targets, so local target ids are unchanged. Renaming a router cascades to its targets; deleting a router still used by targets returns 409.
- A router target produces a local series (no `link` label) plus one series per uplink (`link` label). `PrometheusClient.metrics()` groups by `link` too and keys results by `(host, title, category)`: top-level values are the local series (None if absent), per-link values go in `metrics["links"]`, and `metric_status()` uses the worst loss among local and links.
